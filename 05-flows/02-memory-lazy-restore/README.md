...
VM1、VM2 从同一快照恢复时，内容核心可以合并相同不可变范围的取数任务。内容 ready 后，各自 handler 仍需检查本次区域/页状态，再完成自己的填充；VM1 已写入的页不能被后台结果覆盖，也不能替 VM2 标记“已恢复”。

取消 VM1 只撤销其等待和来源使用关系。VM2 仍需要该任务时继续取数；晚到的 VM1 结果必须被拒绝。后台物化完成与当前缺页完成分别统计，是否继续补齐冷页由恢复策略决定。这些是待验证设计，依据和对照见[QEMU 借鉴分析](../../02-products/09-qemu/03-conch-reference.md)与[数据服务比较](../../03-design-comparison/08-data-service-architecture.md)。
...
4. 两 VM 从同一快照恢复，验证取数复用，同时保持写入、填页状态、身份和运行引用隔离；取消一个不影响另一个。
