...
先保留现有 sparse/bitmap，并为每个范围区分以下状态；换分块文件或日志索引需要单独证明收益。
...
提交顺序保持：**完整写入及所需检查 → cache durability barrier → bitmap ready 写入及同步 → 通知完成。** 批量同步只是后续优化：可以覆盖多个任务，但必须知道哪些范围由同一屏障保护，不能先通知 ready 再补 sync。
...
pmem 主线先验证外部 UFFD。采用后，事件读取、region lookup 与完成关联进入独立适配模块；内容核心仍只处理明确内容范围。若选内部 handler，则沿用范围入口；首版只新增选定方案。文件 remap 必须由 StratoVirt 执行或严格控制，不为了把 handler 外置而扩大 VMM 的内容职责。
