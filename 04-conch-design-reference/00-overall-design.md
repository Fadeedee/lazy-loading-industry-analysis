...
**这是目标流程说明，不是已完成功能清单。** 范围为只读 EROFS + virtio-pmem/DAX，外部 handler（H2）为待验证主方案。本文图中单 sparse 文件与 ready 索引（S1）仍是布局示例。缓存、提交、任务及完成协议统一见[核心选型登记](09-core-design-selection.md)。lazyd 的独立服务模型与扩展边界见[专题设计](../06-lazyd-design/README.md)，CLI、JSON 和 socket 尚未冻结。
...
Conch 将业务场景转换为可选优先级、操作 deadline 和预算；lazyd 决定取数窗口与调度，不解析产品场景；StratoVirt 不学习工作集或选择 registry。三仓使用可关联标识和时间观测，不记录凭据或每次访问的完整明文。
...
| 启动工作集预取 | 调用方提供内容范围提示，lazyd 执行 | 前台优先、字节/并发预算、过时清单可忽略 |
...
| --- | --- | --- |
| 外部 UFFD 与页完成 | 检查交接、相邻 fault、延迟、故障与关闭 | 主方案出现明确阻碍时评估内部备选，不同时建设两套路径 |
