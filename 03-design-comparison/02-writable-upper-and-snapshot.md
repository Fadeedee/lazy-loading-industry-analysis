...
当前三仓设计采用组合 A 的 pmem 只读镜像 + 独立写层。组合 B 保留作技术对照，不是首版并列开发任务；只有遇到具体设备规模、恢复或维护成本问题时才重新评估取舍。
...
先验证当前混合布局的文件正确性、父层查询、保存/恢复耗时、私有写入隔离和业务延迟。需要对照时使用相同工作负载比较完整复制与已有块 COW 后端，不将开发另一套块引擎列为 pmem 前置，也不能只比一次 read 系统调用。
